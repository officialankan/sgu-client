...
def test_station_by_name_platsbeteckning() -> None:
    client = SGUClient()
    station = client.observed_levels.get_station_by_name(
...
def test_station_by_name_no_args() -> None:
    ...
    with pytest.raises(
        ...
        client.observed_levels.get_station_by_name()
...
def test_get_stations_by_names_platsbeteckning() -> None:
    client = SGUClient()
    stations = client.observed_levels.get_stations_by_names(
...
def test_get_stations_by_names_obsplatsnamn() -> None:
    client = SGUClient()
    stations = client.observed_levels.get_stations_by_names(obsplatsnamn=["Lagga_2"])
    assert stations is not None
...
def test_get_stations_by_names_no_args() -> None:
    ...
    with pytest.raises(
        ...
        client.observed_levels.get_stations_by_names()
...
def test_get_stations_by_names_empty_list() -> None:
    ...
    with pytest.raises(
        ...
        client.observed_levels.get_stations_by_names(platsbeteckning=[])
